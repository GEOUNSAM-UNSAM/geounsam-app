  ...
  return (
    <div className="bg-neutral-white rounded-[30px] px-5 py-4 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        <Award size={24} className="text-neutral-dark" />
        <span className="font-saira font-semibold text-lg text-neutral-dark">Insignias</span>
      </div>
      <div className="flex items-stretch justify-between gap-2">
        {insignias.map((ins) => {
          ...
          return (
            ...
            >
              {Icono && <Icono size={24} className={desbloqueada ? 'text-neutral-dark' : 'text-neutral-main'} />}
              <span className={`font-saira text-base ${desbloqueada ? 'text-neutral-dark' : 'text-neutral-main'}`}>{ins.nombre}</span>
              <span className={`font-saira font-medium text-xs text-center ${desbloqueada ? 'text-neutral-dark' : 'text-neutral-main'}`}>{ins.descripcion}</span>
            </div>
          )
        })}
      </div>
